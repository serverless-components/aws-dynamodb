...
class AwsDynamoDb extends Component {
  async deploy(inputs = {}) {
    ...
    log(`Checking if table ${config.name} already exists in the ${config.region} region.`)
  ...
  async remove() {
    ...
    if (this.state.delete === false) {
      log(`Skipping table removal because "delete" is set to "false".`)
      return {}
    }
    ...
    if (!name) {
      log(`Aborting removal. Table name not found in state.`)
      return
    }
    ...
    log(`Removing table ${name} from the ${region} region.`)

    await deleteTable({ dynamodb, name })

    const outputs = pick(outputsList, this.state)

    log(`Table ${name} was successfully removed from the ${region} region.`)
